...
for sub in * ; do
   ...
   run_subj=true
	   if [ "$run_subj" == true ]; then
		...
		sbatch --account=BETHLEHEM-SL2-CPU --nodes=1 --ntasks=1 --cpus-per-task=1 --mem=8000 --time=00:30:00 --output=/rds/project/rb643/rds-rb643-ukbiobank2/logs/${sub}.out /rds/project/rb643/rds-rb643-ukbiobank2/Code/UKB_parcellate.sh ${sub} $SUBJECTS_DIR 
